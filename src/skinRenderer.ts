import { log, lerp, getPixelHex, getPixel, clamp } from "./common";
...
type PoseData = KDict<m4.Vector3>;
...
export class Bone {
    ...
    public transform(mat: m4.Matrix4x4) {
        ...
        cuboids.forEach(cb => {
            ...
            cb.forEach(face => {
                face.forEach(vertex => {
                    var v = [vertex[0], vertex[1], vertex[2], 1] as m4.Vector4;
...
export class SkinRenderer {
    ...
    render() {
        ...
        if (this.poseType == 0) {
            var anim = this.noAnim ? -Math.PI / 4 : lerp(-1, 1, fn(Math.sin(this.seed + performance.now() / animDuration) / 2 + 0.5, 0.25)) * Math.PI / 2.5;
            var headAnim = this.noAnim ? -Math.PI / 4 : lerp(-1, 1, fn(Math.sin(this.seed + performance.now() / animDuration * 2) / 2 + 0.5, 0.25)) * Math.PI / 2.5;
            ...
            var [mx, my] = this.mousePosO ?? [0, 0];
            var yaw = fn(clamp((mx / 12 + 1) / 2, 0, 1), 2.5);
            var pitch = fn(clamp((my / 12 + 1) / 2, 0, 1), 2.5);
            var xRot = lerp(-Math.PI / 2.25, Math.PI / 2.25, pitch);
            var yRot = lerp(Math.PI / 2.25, -Math.PI / 2.25, yaw);
